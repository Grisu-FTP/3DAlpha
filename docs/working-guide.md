...
Start with root `AGENTS.md`. This file is the longer reference, moved from `CLAUDE.md`
without discarding its guidance. Read only the section needed for the current task.
For current implementation notes and dated verification, use `docs/current-work.md`.
...
sanitizers instrument `3dalpha_core`, not just the tests. Dated test results live in `docs/current-work.md`; do not treat a historical count as current verification.
...
| Thing | Where |
|---|---|
| Shell | Use the session’s declared shell; the 2026-09-09 agent session uses bash. Do not assume the user’s interactive shell. |
...
- When a milestone moves or a number is measured, update `docs/status.md` for the detailed record and refresh
  `docs/current-work.md`; run `make index` if files or headings changed.

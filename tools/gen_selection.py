...
def load():
    ...
    **The liquids column varies with metadata and the other does not.**
    ...
    # **The item-render box**, one per block id and not per metadata: the hand
    # and the inventory slot draw `Block.setBlockBoundsForItemRender`'s answer,
    # which has no metadata to consult. It shares the shape list with the
    # selection boxes because it is the same kind of number and three quarters
    # of the rows are already in there.
    item_render = {}
    for values, block in re.findall(r"^    \{([^}]*)\},   // (\d+)", text, re.M)[len(rows):]:
        box = [float(v) for v in values.split(", ")]
        for v in box:
            if struct.unpack("f", struct.pack("f", v))[0] != v:
                sys.exit(f"block {block} item render: {v} does not survive a float")
        if box not in shapes:
            shapes.append(box)
        item_render[block] = shapes.index(box)
    if len(item_render) != 256:
        sys.exit(f"{FIXTURE.name}: {len(item_render)} item-render boxes, expected 256")
    return shapes, index, targetable, liquid_mask, item_render
...
def main():
    shapes, index, targetable, liquid_mask, item_render = load()
    doc = {
        "$comment": [
            ...
            "`itemRender` is Block.setBlockBoundsForItemRender -- the shape the hand and the",
            "inventory slot draw, which has no metadata to read and so is one index per",
            "block. It is the constructor's bounds for every block but the button and the",
            "two pressure plates, whose classes override the method.",
        ],
        "version": "a1.1.2",
        "shapes": shapes,
        "index": index,
